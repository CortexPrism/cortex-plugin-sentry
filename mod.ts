  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const data = await sentryRequest(
        `/projects/${config.sentryOrg}/${project}/issues/?query=${
...
const sentry_analyze_stacktrace: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!issueId) {
        return {
          toolName: 'sentry_analyze_stacktrace',
          success: false,
          output: '',
          error: 'issue_id is required',
          durationMs: Date.now() - start,
      ...
      const events = await sentryRequest(`/issues/${issueId}/events/?limit=1`) as Record<
        string,
        unknown
      >[];
      ...
      if (!stacktraceEntry) {
        return {
          toolName: 'sentry_analyze_stacktrace',
          success: true,
          output: 'No stacktrace available for this event',
          durationMs: Date.now() - start,
        };
      }

      const frames =
        ((stacktraceEntry.data as Record<string, unknown>).frames as Record<string, unknown>[]) ||
        [];
      ...
      const analysis = {
        ...
        topFrames: (inAppFrames.length > 0 ? inAppFrames : frames).slice(0, 5).map((
      ...
      return {
        ...
        success: false,
        output: '',
        error: `Failed to analyze stacktrace: ${
...
const sentry_create_github_issue: Tool = {
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!issueId || !repo) {
        return {
          toolName: 'sentry_create_github_issue',
          success: false,
          output: '',
          error: 'issue_id and repo are required',
          durationMs: Date.now() - start,
        };
      }
      ...
      if (!ghRes.ok) {
        return {
          ...
          success: false,
          output: '',
          error: `GitHub API: ${ghData.message}`,
          durationMs: Date.now() - start,
        };
      }
      ...
      return {
        toolName: 'sentry_create_github_issue',
        success: true,
        output: `GitHub issue created: ${ghData.html_url}`,
        durationMs: Date.now() - start,
      };
    } catch (error) {
      return {
...
export const tools: Tool[] = [
  sentry_list_issues,
  sentry_get_issue,
  sentry_analyze_stacktrace,
  sentry_suggest_fix,
  sentry_create_github_issue,
];
